...
struct BitVecNums<simde__m128i, typename std::enable_if<true>::type> {
  ...
  static inline simde__m128i zero_block() { return simde_mm_setzero_si128(); }
  ...
  inline static uint64_t popcount(simde__m128i a) {
    ...
    uint64_t hw =
        simde_mm_extract_epi64(pops, 0) + simde_mm_extract_epi64(pops, 1);
    return hw;
  }
...
struct BitVecNums<simde__m256i, typename std::enable_if<true>::type> {
  ...
  inline static uint64_t popcount(simde__m256i a) {
    uint64_t hw =
        BitVecNums<uint64_t>::popcount(simde_mm256_extract_epi64(a, 0)) +
...
template <> inline bool nzpopblk<simde__m128i, uint8_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);

  simde__m128i zblocks =
      simde_mm_cmpeq_epi8(a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm_testz_si128(zblocks, ones) == 0;
}

template <> inline bool nzpopblk<simde__m128i, uint16_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);

  simde__m128i zblocks =
      simde_mm_cmpeq_epi16(a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm_testz_si128(zblocks, ones) == 0;
}

template <> inline bool nzpopblk<simde__m128i, uint32_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);

  simde__m128i zblocks =
      simde_mm_cmpeq_epi32(a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm_testz_si128(zblocks, ones) == 0;
}

template <> inline bool nzpopblk<simde__m128i, uint64_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);

  simde__m128i zblocks =
      simde_mm_cmpeq_epi64(a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm_testz_si128(zblocks, ones) == 0;
}

template <> inline bool nz2kpopblk<simde__m128i, uint8_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);
  simde__m128i am1 = simde_mm_sub_epi8(a, simde_mm_set1_epi8(1));
  simde__m128i tst = simde_mm_and_si128(a, am1);
  simde__m128i zblocks = simde_mm_cmpeq_epi8(
      tst, zeros); // blocks with 0 or 1 HW are all-ones blocks
  return simde_mm_testz_si128(zblocks, ones) == 0;
}

template <> inline bool nz2kpopblk<simde__m128i, uint16_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);
...
template <> inline bool nz2kpopblk<simde__m128i, uint32_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);
...
template <> inline bool nz2kpopblk<simde__m128i, uint64_t>(simde__m128i a) {
  simde__m128i zeros = BitVecNums<simde__m128i>::zeros;
  simde__m128i ones = simde_mm_cmpeq_epi64(zeros, zeros);
...
template <> inline bool nzpopblk<simde__m256i, uint8_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);

  simde__m256i zblocks = simde_mm256_cmpeq_epi8(
      a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm256_testz_si256(zblocks, ones) == 0;
}

template <> inline bool nzpopblk<simde__m256i, uint16_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);

  simde__m256i zblocks = simde_mm256_cmpeq_epi16(
      a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm256_testz_si256(zblocks, ones) == 0;
}

template <> inline bool nzpopblk<simde__m256i, uint32_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);

  simde__m256i zblocks = simde_mm256_cmpeq_epi32(
      a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm256_testz_si256(zblocks, ones) == 0;
}

template <> inline bool nzpopblk<simde__m256i, uint64_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);

  simde__m256i zblocks = simde_mm256_cmpeq_epi64(
      a, zeros); // blocks with zero HW are all-ones blocks
  return simde_mm256_testz_si256(zblocks, ones) == 0;
}

template <> inline bool nz2kpopblk<simde__m256i, uint8_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);
...
template <> inline bool nz2kpopblk<simde__m256i, uint16_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);
...
template <> inline bool nz2kpopblk<simde__m256i, uint32_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);
...
template <> inline bool nz2kpopblk<simde__m256i, uint64_t>(simde__m256i a) {
  simde__m256i zeros = BitVecNums<simde__m256i>::zeros;
  simde__m256i ones = simde_mm256_cmpeq_epi64(zeros, zeros);
