...
//! BitVec and BitMatrix are classes that wrap around bit arrays with
//! convenience functions for individual bit access and manipulation. They are
//! parametrized by a base unsigned integer type as the block size.
...
template <typename T, typename _enabled = void> struct BitVecNums;
...
template <typename T, typename U> inline bool nzpopblk(T a) {
...
template <typename T, typename U> inline bool nz2kpopblk(T a) {
  // Treating a as an array of packed U types
...
template <typename T> class BitVecSlice {
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const BitVecSlice<T> &slice) {
    std::ios::fmtflags os_flags(os.flags());
    os << std::setfill('0') << std::hex;
    for (size_t bi = slice.begin; bi < slice.end; ++bi) {
      os << std::setw(std::numeric_limits<T>::digits / 4)
         << (uint64_t)slice.get_block(bi) << ' ';
    }
    os << '\n';
    os.flags(os_flags);
    return os;
  ...
  [[nodiscard]] size_t hamming_weight() const {
    ...
    /// Sums the number of ones in every block of the array, including the
    /// unused bits, so this is only correct as long as no invalid bits are set
    /// in the last block.
    size_t hw = 0;
...
template <typename T> class BitVec {
...
public:
  ...
  ~BitVec() { operator delete[](data, std::align_val_t(alignment)); }
  static const int16_t bits_per_block = BitVecNums<T>::bits_per_block;
  ...
  void clear() { std::fill(data, data + nblocks, BitVecNums<T>::zero_block()); }
  uint64_t num_blocks() { return nblocks; }
...
template <typename T> struct BitMatrix {
  ...
  BitMatrix(const BitMatrix &mat)
      : rows(mat.rows), row_blocks(mat.row_blocks), cols(mat.cols) {
  ...
  ~BitMatrix() { operator delete[](data, std::align_val_t(alignment)); }
  ...
  T &get_block(uint64_t bi, uint64_t j) {
    ...
    if (j >= cols || bi >= row_blocks) {
      std::stringstream ss;
      ss << "BitMatrix::get_block() index error. Index (" << bi << ", " << j
         << " ) out of bounds "
         << "( " << row_blocks << ", " << cols << ").";
