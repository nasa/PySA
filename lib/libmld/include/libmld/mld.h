...
public:
    ...
    [[nodiscard]] const std::vector<std::vector<mld_uint_t>>& clause_list() const{
        return clauses;
    }

    [[nodiscard]] const std::vector<uint8_t>& yenc() const{
        return yarr;
    }

    template<typename T>
    BitMatrix<T> clauses_as_bitmatrix(bool y_aug=true){
        BitMatrix<T> bm(nrows, nvars+(y_aug?1:0));
        for(size_t i = 0; i < nrows; ++i ){
            for(mld_uint_t j : clauses[i]){
                bm.set(i, j, true);
            }
            if(y_aug && yarr[i]>0) bm.set(i, nvars, true);
        }
        return bm;
    }
    ...
    BinMatC clauses_as_binmat(bool y_aug=true){
        ...
        for(size_t i = 0; i < nrows; ++i ){
            ...
            if(y_aug && yarr[i]>0) bm(i, nvars) = 1;
        }
        return bm;
    }

    template<typename T>
    BitVec<T> y_as_bitvec(){
        BitVec<T> yv(nrows);
        for(size_t i = 0; i < nrows; ++i){
            if(yarr[i] > 0) yv.set(i, true);
        }
        return yv;
    }
