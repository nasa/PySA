...
public:
    ...
    void shuffle_isd() {
        std::shuffle(info_set.begin(), info_set.end(), rng);
    };
...
//                T *Gpi = &G.get_block(0, coli);
    ...
    const BitVec<T> &get_solution_vec() const {
        return solution_vec;
    }
...
protected:
    void finalize_solution(size_t isd1, size_t isd2){
        ...
        for(size_t bi = 0; bi < work_vec.num_blocks(); ++bi){
            work_vec.get_block(bi) = Gi.get_block(bi) ^ Gj.get_block(bi) ^ G.get_block(bi, n);
        }
        // The solution has the desired hamming weight with w-2p errors in the redundancy set
        solution_vec.clear();
    ...
    std::vector<size_t> pivt_cols; // Pivot columns of gaussian elimination == redundancy set
