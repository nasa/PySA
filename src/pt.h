...
namespace ptxor {
template <typename B>
int64_t energy(const BitMatrix<B> &G, const BitVec<B> &y, const BitVec<B> &x,
               BitVec<B> &Gx) {
#ifndef NDEBUG
  if (G.cols != x.size()) {
...
template <typename B> class PTSampler {
public:
  PTSampler(const BitMatrix<B> &G, const BitVec<B> &y,
            std::vector<float> &beta_array, pt_rng_t &&rng,
            std::vector<BitVec<B>> &&x_arr, int64_t target_energy = 0)
      : G(G), y(y), beta_array(beta_array), x_arr(x_arr), rng(rng),
  ...
  std::vector<int64_t> &current_energies() { return energies; }
  std::vector<BitVec<B>> &current_states() { return x_arr; }
  std::vector<uint64_t> &acceptances() { return num_acceptances; }
