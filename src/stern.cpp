...
template<typename stern_uint_t, typename sub_block_t=void, bool testhw1=false>
void sterncpp(MLDProblem& mld_problem, sternc_opts& opts){
    ...
    if(rank == master_rank){
        if constexpr (std::is_void_v<sub_block_t>)
            std::cout << "Sterncpp / Block bits " << sizeof(stern_uint_t)*8<< " / Heavy Stern\n" ;
        else {
        ...
        std::cout << "Alignment: " << BitVecNums<stern_uint_t>::alignment << " bytes.\n";
    }
    ...
    BitMatrix<stern_uint_t> hy = mld_problem.clauses_as_bitmatrix<stern_uint_t>(true);
    BitVec<stern_uint_t> stern_solution(n);
    SternP1<stern_uint_t> stern_p1(hy, n, k, rng);
#ifdef USEMPI
    if(rank==master_rank){
        MPI_Irecv(compl_buf, 3, MPI_INT64_T, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD,&mpireq);
    } else {
    ...
    for(int32_t i = 0; i < opts.max_iters; ++i){
        ...
        if(!opts.bench){
            if(rank==master_rank){
                ...
                if(mpiflag){
                    if(compl_buf[2] > 0){ // broadcast the successful completion and terminate
                        std::copy(compl_buf, compl_buf+3, complt);
                        MPI_Ibcast(complt, 3, MPI_INT64_T, master_rank, MPI_COMM_WORLD, &mpireq2);
...
#endif
        if(mfound){
            if(nsuccess==0) stern_solution = stern_p1.get_solution_vec();
            complt[0] = rank;
            complt[1] = i;
            complt[2] = 1;
            nsuccess++;
    ...
    if(opts.bench){
        ...
        MPI_Allreduce(&nsuccess, &nsuccess_buf, 1, MPI_UINT32_T, MPI_SUM, MPI_COMM_WORLD);
        niters = opts.max_iters * nproc;
        if(nsuccess_buf > 0){ // transmit the solution to the master rank from the first rank to find it
            MPI_Allgather(&nsuccess, 1, MPI_UINT32_T, rank_nsuccesses.data(), 1, MPI_UINT32_T, MPI_COMM_WORLD);
            for(int i = 0; i < nproc; ++i){
                if(rank_nsuccesses[i] > 0){
                    if(i!=master_rank){ // ping pong
                        if(rank == master_rank)
                            MPI_Recv(stern_solution.data_ptr(), (int)stern_solution.num_blocks(), STERN_MPI_INT, i, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        if(rank == i)
                            MPI_Send(stern_solution.data_ptr(), (int)stern_solution.num_blocks(), STERN_MPI_INT, master_rank, 2, MPI_COMM_WORLD);
                        if(rank == master_rank)
                            MPI_Recv(complt, 3, MPI_INT64_T, i, 3, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        if(rank == i)
        ...
        // synchronize one more time to break if two ranks found a solution simultaneously before the master rank could broadcast
        MPI_Bcast(complt, 3, MPI_INT64_T, master_rank, MPI_COMM_WORLD);
        MPI_Bcast(stern_solution.data_ptr(), (int) stern_solution.num_blocks(), STERN_MPI_INT, complt[0], MPI_COMM_WORLD);
        MPI_Reduce(&niters, &niters_buf,1, MPI_UINT32_T, MPI_SUM, master_rank, MPI_COMM_WORLD);
        if(rank==master_rank && complt[2]>0){
            std::cout << "Solution found in rank " << complt[0] << " Iter " << complt[1] << std::endl;
        }
    }
    MPI_Reduce(&dt, &dt_sum, 1, MPI_UINT64_T, MPI_SUM, master_rank, MPI_COMM_WORLD);
#else
    dt_sum = dt;
    niters_buf = niters;
#endif
...
void sterncpp_switch(MLDProblem& mld_problem, sternc_opts& opts, size_t block_size){
    // sorry
    switch(block_size){
        case 8: // uint8_t
            switch(opts.l){
...
bool sterncpp_adjust_opts(sternc_opts& opts, size_t& block_size){
    ...
    if(opts.m > 0 && opts.l > 0){ // determine block size from passed -l and -m options
...
//            case 256:
            ...
            default:
                if(mpi_rank==0)
                    std::cout << "Option --block-size " << opts.block_size << " not valid.\n" ;
                return false;
        }
