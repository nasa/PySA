...
int main(int argc, char** argv) {
    ...
    desc.add_options()
                ("help,h", "Help screen")
                ("bench", bool_switch(&bench), "Continue until max_iters iterations and count how often the solution was found." )
                ("sterncpp", bool_switch(&use_sterncpp), "Use the optimized sterncpp implementation. Otherwise, runs the reference Fortran implementation.")
                ("test-hw1", bool_switch(&use_hw1), "Check for Hamming weight <= 1 in collision bits")
                ("max_iters", value<int32_t>(&maxiters), "Maximum number of iterations per rank. Overrides max_factor.")
                ("block-size", value<int32_t>(&opts.block_size), "Block size of parity check bit columns.")
                ("col-size,l", value<int32_t>(&l), "Size of collision set to check")
                ("col-sets,m", value<int32_t>(&m), "Number of collision sets to check")
                ("combo-size,p", value<int32_t>(&p), "Size of information set combinations. Only p=1 is supported in sterncpp.")
                ("max_factor", value<int32_t>(&max_factor)->default_value(10), 
                "Set the maximum number of iterations across all ranks to max_factor * (2**P), where P is the estimated success probability.")
    ...
    if(l>0){
        ...
        if(mpi_rank==0) std::cout << "-- Heavy Stern --\n";
    ...
    if(mpi_rank == 0){
        ...
        std::cout << "(3) Expected work to success = (1b)+(2) : " << iterwork + success_bits << " bits" << std::endl;
        std::cout << "(4) lg NTS(99%) : " << nts << " bits\n";
        std::cout << "(5) lg TTS(99%) = (2)+(4) : " << nts + iterwork << " bits\n\n";
