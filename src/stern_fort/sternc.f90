...
contains
  subroutine cptr_to_arr(cparr, farr)
    ! Copy the contents of a C array containing a row-major matrix
    ! into a 2D fortran array
  ...
  subroutine into_parcheck(codearr, opts, newarr, iscols)
    use lincode
    ! Convert a generator matrix problem into a parity check problem,
    ! if not already one
...
subroutine sternc(cinarr, sternopts)
  ...
  ! input options converted to parity check form, if necessary
  type(SterncOpts) opts
  integer(kind = c_int32_t) :: numvars, numclauses, t
  integer(kind = c_int32_t) :: max_iters, l_, p, m
  integer k
  integer l
  !integer(8) memsize
  ! CPU time start and end, rank CPU time, and total MPI CPU time
  real :: tstart, tend, dt, dt_buf
  ! success probability, time per iteration, time to solution
  real(8) :: psucc, tperit, tts
  ...
  if (rank == master_rank) then
    call MPI_Irecv(compl_buf, 3, MPI_INTEGER8, MPI_ANY_SOURCE, 1, &
        MPI_COMM_WORLD, mpireq, ierr)
  else
    call MPI_Ibcast(compl_buf, 3, MPI_INTEGER8, master_rank, &
        MPI_COMM_WORLD, mpireq, ierr)
  end if
#endif
  compl(1) = 0
  ...
  do i = 1, max_iters
    ...
    if(opts%bench==0) then
      if(rank == master_rank) then
        ...
        if(mpiflag) then
          if(compl_buf(3) > 0) then
            ! broadcast successful completion from another rank and terminate
            compl(:) = compl_buf(:)
            call MPI_Ibcast(compl, 3, MPI_INTEGER8, master_rank, &
                MPI_COMM_WORLD, mpireq2, ierr)
...
#endif
    ! There can be a deadlock condition if the master rank and a worker rank
    ! both simultanously find the solution, check MPI_Test, and then worker
    ! hangs on MPI_Send while master hangs on MPI_Wait. This is extremely
    ! unlikely but might be worth addressing.
    if(mfound > 0) then
      ...
      if(rank == master_rank .and. opts%bench==0) then
        call MPI_Ibcast(compl, 3, MPI_INTEGER8, master_rank, MPI_COMM_WORLD, &
            mpireq2, ierr)
        call MPI_Wait(mpireq2, MPI_STATUS_IGNORE, ierr)
  ...
  if(opts%bench>0) then
    call MPI_Allreduce(nsuccess, nsucc_buf, 1, MPI_INTEGER8, MPI_SUM, &
        MPI_COMM_WORLD, ierr)
    !nsuccess = nsucc_buf
    niters = max_iters * nproc
    if(nsucc_buf>0) then ! transmit the solution to the master rank
      allocate(rank_nsuccesses(nproc))
      call MPI_Allgather(nsuccess, 1, MPI_INTEGER, rank_nsuccesses, 1, &
          MPI_INTEGER, MPI_COMM_WORLD, ierr)
      do i = 0, nproc-1
        if(rank_nsuccesses(i+1)>0) then
          if(i/=master_rank) then
            if(rank==master_rank)  &
                call MPI_Recv(stern_solution, numvars, MPI_INTEGER1, i, 2, &
                    MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
            if(rank==i)  &
                call MPI_Send(stern_solution, numvars, MPI_INTEGER1, &
                    master_rank, 2, MPI_COMM_WORLD, ierr)
            if(rank==master_rank) &
                call MPI_Recv(compl, 3, MPI_INTEGER8, i, 3, MPI_COMM_WORLD, &
                    MPI_STATUS_IGNORE, ierr)
            if(rank==i) &
                call MPI_Send(compl, 3, MPI_INTEGER8, master_rank, 3, &
                    MPI_COMM_WORLD, ierr)
          end if
          exit
        end if
    ...
    call MPI_Bcast(stern_solution, numvars, MPI_INTEGER1, int(compl(1), 4), &
        MPI_COMM_WORLD, ierr)
    call MPI_Bcast(isd_sol, k, MPI_INTEGER, int(compl(1), 4), &
        MPI_COMM_WORLD, ierr )
    call MPI_Reduce(niters, niters_buf, 1, MPI_INTEGER8, MPI_SUM, &
        master_rank, MPI_COMM_WORLD, ierr)
    if(rank==master_rank .and. compl(3)>0)  &
        print '(A,I10,A,I10)', "Solution found in Rank ", compl(1), &
            " Iter ", compl(2)
  end if
  call MPI_Reduce(dt, dt_buf, 1, MPI_REAL4, MPI_SUM, master_rank, &
      MPI_COMM_WORLD, ierr)
#else
  dt_buf = dt
  niters_buf = niters
