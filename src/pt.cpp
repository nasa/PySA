...
template <typename B> struct PTRunner {
  BitMatrix<B> G;
  BitMatrix<B> y;
...
template <typename B>
void run_pt(MLDProblem &mld_problem, float beta_min, float beta_max,
            size_t num_temperatures, size_t max_sweeps, uint64_t seed = 0) {
  ...
  double lgbmin = std::log(double(beta_min)),
         lgbmax = std::log(double(beta_max));
...
_a:
  ...
  for (size_t j = 0; j < num_temperatures; ++j) {
    std::cout << "PT " << j << ' ' << beta_arr[j] << ' ' << energies[j] << ' '
              << sampler.acceptances()[j] << '\n';
  }
  for (size_t j = 0; j < num_temperatures; ++j) {
    if (energies[j] <= energy_target) {
      ...
      std::cout
          << ti.format(
                 6, "wall %w s | user %u s | system %s s | CPU %t s | (%p%)\n")
          << std::endl;
...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help,h", "Help screen")(
      "beta-min", value<float>(&beta_min))("beta-max", value<float>(&beta_max))(
      "num-temps", value<unsigned>(&num_temperatures))(
      "input", value<std::string>(&filenm))("max-sweeps",
                                            value<size_t>(&max_sweeps));
  posopts.add("input", 1);
  ...
  if (m <= 8) {
    run_pt<uint8_t>(problem, beta_min, beta_max, num_temperatures, max_sweeps,
                    seed);
  } else if (m <= 16) {
    run_pt<uint16_t>(problem, beta_min, beta_max, num_temperatures, max_sweeps,
                     seed);
  } else if (m <= 32) {
    run_pt<uint32_t>(problem, beta_min, beta_max, num_temperatures, max_sweeps,
                     seed);
  }
#if defined(USE_SIMDE)
  else if (m <= 64) {
    run_pt<uint64_t>(problem, beta_min, beta_max, num_temperatures, max_sweeps,
                     seed);
  } else {
    run_pt<simde__m128i>(problem, beta_min, beta_max, num_temperatures,
                         max_sweeps, seed);
  }
#else
  else {
