...
def _get_energy(w):
    ...
    with tqdm(states, total=verbose, disable=not verbose) as pbar:
        ...
        for state in pbar:
            ...
            energy = qubo.get_energy(couplings, local_fields, state)
...
def test_update_spin_qubo(n_vars: int):

    # Generate random problem
    couplings, local_fields = gen_random_problem(n_vars, dtype=dtype)

    # Get initial state
    state = np.random.randint(2, size=n_vars).astype(dtype)
    ...
    for pos in np.random.permutation(n_vars):
        ...
        de = qubo.update_spin(couplings, local_fields, state, pos, 0, -1)
        ...
        assert (np.isclose(energy + de,
                           qubo.get_energy(couplings, local_fields, state)))
...
@pytest.mark.parametrize(
    'n_vars', [n_vars for n_vars in range(32, 129, 32) for _ in range(20)])
def test_sequential_sweep_qubo(n_vars: int):
    ...
    de = simulation.sequential_sweep(qubo.update_spin, couplings, local_fields,
                                     state, 0)
...
@pytest.mark.parametrize(
    'n_vars', [n_vars for n_vars in range(32, 129, 32) for _ in range(20)])
def test_random_sweep_qubo(n_vars: int):
    ...
    # Apply sweep (infinite temperature)
    de = simulation.random_sweep(qubo.update_spin, couplings, local_fields,
                                 state, 0)
...
def test_pt(n_vars: int, n_replicas: int):
    ...
    assert (np.alltrue([
        np.isclose(energies[k],
                   qubo.get_energy(couplings, local_fields, states[k]))
...
def test_random_sweep_simulation_qubo(n_vars: int):
    ...
    assert (np.isclose(best_energy,
                       qubo.get_energy(couplings, local_fields, best_state)))
