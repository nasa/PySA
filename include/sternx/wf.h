...
inline double heavy_stern_iterwf(int n, int k, int p, int l = 0) {
  ...
  //
  // Operation:  Diagonalize the parity check matrix with Gaussian elimination
  // ((n-k)^3). For each of I1 and I2, construct every p-linear combination of
  // columns (about 2*((k/2)Cp)*(p * (n-k)) For each p-combination v1 from I1,
  // For each p-combination v2 from I2, evaluate the Hamming distance of their
  // sum from y ( (k/2Cp)*(k/2Cp)*(n-k))

  double ge_work_bits = 3 * std::log2(n - k);
  double lc_work_bits =
      1.0 + log2_binomial(k / 2, p) + std::log2(p) + std::log2(n - k);
  double q_work_bits;
  if (l > 0)
  ...
  double total_bits =
      log2sumexp(log2sumexp(ge_work_bits, lc_work_bits), q_work_bits);
  return total_bits;
}
...
inline double heavy_stern_nll(int n, int k, int w, int p) {
  ...
  double wf_infoset = log2_binomial(n, k) - log2_binomial(w, 2 * p) -
                      log2_binomial(n - w, k - 2 * p);
  double cond_wf_split =
      -(log2_binomial(k / 2, p) + log2_binomial(k - k / 2, p) -
        log2_binomial(k, 2 * p));
...
inline double stern_nll(int n, int k, int w, int p, int l, int m) {
  ...
  // Next, we want to calculate the number of ways to distribute (n-k) bits with
  // w-2p error bits into m buckets of l bits such that (X) at least one bucket
  // contains zero [or one] error bits.
  ...
  // One random selection of l bits from the redundancy contains at least one
  // error
  double b1inv = -std::log1p(-std::exp2(-b1)) / std::log(2.0);
  // m independent random selections of l bits from the redundancy set contain
  // at least one error
  double b2 = m * b1inv;
  // at least one out of m independent random selections of l bits from the
  // redundancy set contains no errors
