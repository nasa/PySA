...
struct sternc_opts {
  ...
  /// the generator matrix transpose.
  int8_t parcheck = 0;
  ...
  int8_t bench = 0;
  /// (C, Experimental) Allow a collision with a Hamming weight of 1 out of l
  /// rows to trigger a sum across all n-k rows in the standard Stern algorithm.
  /// Ignored in the heavy Stern algorithm.
  int8_t test_hw1 = false;
  /// (F) Number of columns in the matrix. If parcheck>0, this is the number of
  /// colmuns of the parity check matrix n. Otherwise, it is the number of
  /// columns of the generator matrix transpose k.
  int32_t nvars = 0;
  /// (F) Number of rows in the matrix. If parcheck>0, this is the number of
  ...
  int32_t max_iters;
  /// (F, C) Size of collision set to check. (Ignored in the heavy Stern
  /// algorithm.) (C) Must be chosen from 8, 16, or 32
  int32_t l = 0;
  /// (F) Size of information set combinations. (Only p=1 is supported in
  /// sterncpp.)
  int32_t p = 1;
  /// (F, C) Number of collision sets to check. (Ignored in the heavy Stern
  /// algorithm.)
  int32_t m = 1;
  /// (C) Integer or SIMD block size in bits. Must be chosen from 8, 16, 32,
