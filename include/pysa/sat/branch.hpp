...
struct Branch {
  ...
  Branch(InstancePtr instance, std::size_t max_n_unsat = 0)
      : instance{instance}, max_n_unsat{max_n_unsat},
        _state(instance->n_vars()), _partial_sat(instance->n_clauses()),
        _pos{0}, _n_unsat{0}, _valid{true} {}
  ...
  Branch(InstancePtr instance, std::size_t max_n_unsat, BitSet state,
      ...
      : instance{instance}, max_n_unsat{max_n_unsat}, _state{state},
        _partial_sat{partial_sat}, _pos{pos}, _n_unsat{n_unsat}, _valid{valid} {
  }
  ...
  template <typename Buffer> auto dump() const {
    /*
     * Dump Branch.
     */
...
template <typename StateType, typename SizeType> struct Configuration {
  StateType state;
  SizeType n_unsat;
...
struct Archiver<pysa::dpll::sat::Branch<BitSet, List, Vector, Instance>> {
  ...
  static constexpr auto loads(typename Buffer::const_iterator buffer,
    ...
    return std::pair{h6_, base_type(instance, max_n_unsat_, std::move(state_),
                                    std::move(partial_sat_), pos_, n_unsat_,
                                    valid_)};
  }
};
...
struct Archiver<
  ...
  static constexpr auto dumps(const std::list<base_type> &branches) {
    ...
    // Dump branches
    for (const auto &br_ : branches)
      buffer_ += dump<Buffer>(br_);
